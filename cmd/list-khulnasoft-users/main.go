...
func main() {
	logger := log.Output(zerolog.ConsoleWriter{Out: os.Stderr}).Level(zerolog.InfoLevel).With().Str("program", "list-khulnasoft-users").Logger()
...
func core(logger zerolog.Logger) error {
	...
	defer stop()
	gh := NewGitHub(ctx)
	// query := "-user:sulaiman-coder -org:khulnasoftproj khulnasoftproj"
	query := "khulnasoftproj/khulnasoft-registry"
...
func (gh *GitHub) GetRepo(ctx context.Context, repoOwner, repoName string) (Repo, error) {
	/*
	  repository(owner: "sulaiman-coder", name: "tfcmt") {
	    stargazerCount
	  }
	*/
	var q struct {
